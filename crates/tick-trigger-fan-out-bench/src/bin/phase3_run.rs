...
use tick_trigger_fan_out_bench::senders::{
    allenhark::AllenHarkSender,
    astralane::AstralaneSender,
    astralane_quic::AstralaneQuicSender,
    blockrazor::BlockRazorSender,
    bloxroute::BloxrouteSender,
    bloxroute_quic::BloxrouteQuicSender,
    helius::HeliusSender,
    jito::{tip_updater::JitoTipUpdater, JitoBundleCounters, JitoBundleSender},
    nextblock::NextblockSender,
    nextblock_quic::NextblockQuicSender,
    nozomi::NozomiSender,
    syncro::SyncroSender,
    triton::TritonSender,
    triton_sendtx::TritonSendTxSender,
    zeroslot::ZeroSlotSender,
    TxSender,
};
...
fn main() -> anyhow::Result<()> {
    ...
    for sc in &enabled_senders {
        let s: Arc<dyn TxSender> = match sc.kind {
            ...
            SenderKind::TritonSendTx => {
                // New `sendtx` direct-HTTP fast path. `api_key` (if set) is the
                // `x-token` header; otherwise the token is in `endpoint_url`.
                let t = TritonSendTxSender::new(
                    sc.id,
                    sc.name.clone(),
                    sc.endpoint_url.clone(),
                    sc.api_key.clone(),
                );
                // Pre-warm the keep-alive connection off the hot path.
                t.spawn_warmup(&bg_handle);
                Arc::new(t) as Arc<dyn TxSender>
            }
            SenderKind::Nozomi => {
                // Multi-region fan-out: same signed tx → N regional hosts,
                // first reply wins. `api_key` is the `?c=` query auth; one
                // in-tx tip (>=1_000_000 lamports) serves all regions.
                ...
                // Pre-warm one keep-alive connection per region off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::Astralane => {
                // Iris /irisb raw-octet fast path. `api_key` is the `?api-key=`
                // query auth; `outbound_ips` bind sends to the whitelisted FRA
                // egress IPs; one in-tx tip serves all fan-out regions.
                ...
                // Pre-warm one keep-alive connection per (region, source IP).
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::AstralaneQuic => {
                // Raw QUIC fast path. `AstralaneQuicSender::new` builds a quinn
                // endpoint (needs an ambient Tokio runtime). `api_key` becomes
                // the client-cert CN; `outbound_ips[0]` pins the (whitelisted)
                // egress IP for the UDP socket.
                let _rt_guard = bg_handle.enter();
                ...
                .with_context(|| format!("build astralane_quic sender {:?}", sc.name))?;
                // Pre-warm the persistent QUIC connection off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::Nextblock => {
                // NextBlock HTTP REST /api/v2/submit. `api_key` is the raw
                // `Authorization` header (no Bearer); `outbound_ips` bind source
                // IPs; mandatory in-tx tip (>=1_000_000 lamports).
                ...
                // Pre-warm the keep-alive connection(s) off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::NextblockQuic => {
                // Raw QUIC (:11100, ALPN nb-tx/1). `NextblockQuicSender::new`
                // builds a quinn endpoint (needs an ambient Tokio runtime);
                // `api_key` is the byte-handshake auth; `outbound_ips[0]` pins
                // the (whitelisted) egress IP.
                let _rt_guard = bg_handle.enter();
                ...
                .with_context(|| format!("build nextblock_quic sender {:?}", sc.name))?;
                // Pre-warm + auth the persistent QUIC connection off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::Bloxroute => {
                // bloXroute Solana Trader API /api/v2/submit over plaintext HTTP,
                // multi-region fan-out (germany + global + ...). `api_key` is the
                // raw `Authorization` token; one in-tx tip serves all regions.
                ...
                // Pre-warm keep-alive connection(s) per (region, source IP).
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::BloxrouteQuic => {
                // bloXroute raw QUIC (mTLS), multi-region fan-out (germany +
                // global). `BloxrouteQuicSender::new` builds a quinn endpoint
                // (needs an ambient Tokio runtime) and loads the portal mTLS
                // PEMs (`tls_cert_path` + `tls_key_path`). `outbound_ips[0]`
                // pins the egress IP.
                let _rt_guard = bg_handle.enter();
                ...
                .with_context(|| format!("build bloxroute_quic sender {:?}", sc.name))?;
                // Pre-warm + mTLS-verify each regional QUIC connection off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::Syncro => {
                let s = SyncroSender::new(
                    sc.id,
                    sc.name.clone(),
                    sc.endpoint_url.clone(),
                    sc.api_key.clone(),
            ...
            SenderKind::ZeroSlot => {
                ...
                // Pre-warm the keep-alive connection (GET /health) off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::AllenHark => {
                // `AllenHarkSender::new` builds a quinn endpoint via
                // `quinn::Endpoint::client()`, which requires an ambient Tokio
                // runtime. The factory runs in sync `fn main()` (only a Handle is
                // held), so enter the background runtime for the construction.
                let _rt_guard = bg_handle.enter();
                let s = AllenHarkSender::new(
                    sc.id,
                    sc.name.clone(),
                    sc.endpoint_url.clone(),
                    sc.api_key.clone(),
                )
                .with_context(|| format!("build allenhark sender {:?}", sc.name))?;
                // Pre-warm the persistent QUIC connection off the hot path.
                s.spawn_warmup(&bg_handle);
                Arc::new(s) as Arc<dyn TxSender>
            }
            SenderKind::BlockRazor => {
                let s = BlockRazorSender::new(
                    sc.id,
                    sc.name.clone(),
                    sc.endpoint_url.clone(),
                    sc.api_key.clone(),
    ...
    println!(
        "{:<3} {:<14} {:>5} {:>5} {:>6} {:>7} {:>7} {:>12} {:>13}",
        "id", "name", "wins", "sent", "win%", "share%", "blocked", "rtt_avg(us)", "obs_avg(us)"
    );
    ...
    for ps in &active.per_sender {
        // `sent` = txs that actually reached the network (landed + lost-but-sent).
        // `win%` = wins / sent - the fair per-attempt win-rate, EXCLUDING
        // throttled/rate-limited sends (`blocked` = send_error), so senders that
        // send different volumes are comparable. `share%` = share of all landings.
        let sent = ps.landed + ps.unknown_pending;
