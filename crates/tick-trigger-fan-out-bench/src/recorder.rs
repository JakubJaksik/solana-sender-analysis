...
impl PerSenderStats {
    ...
    /// Win-rate normalised by attempts that actually reached the network:
    /// `landed / (landed + unknown_pending)`. `unknown_pending` = sent but did
    /// not win; `send_error` (throttled / rate-limited = never competed) is
    /// EXCLUDED. `None` when the sender never got a tx out. This is the fair
    /// per-attempt win-rate when senders send different numbers of txs.
    pub fn win_rate_pct(&self) -> Option<f64> {
        let sent = self.landed + self.unknown_pending;
        if sent == 0 {
            None
        } else {
            Some(self.landed as f64 / sent as f64 * 100.0)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn win_rate_pct_normalises_by_sent_and_excludes_send_errors() {
        // 30 landed, 70 sent-but-lost (sent=100); 50 throttled/errored excluded.
        let ps = PerSenderStats {
            sender_id: 0,
            attempts: 150,
            landed: 30,
            send_error: 50,
        ...
        assert_eq!(ps.win_rate_pct(), Some(30.0));
    }

    #[test]
    fn win_rate_pct_none_when_nothing_reached_network() {
        let ps = PerSenderStats {
            sender_id: 0,
            attempts: 10,
            landed: 0,
        ...
        assert_eq!(ps.win_rate_pct(), None);
    }
}
